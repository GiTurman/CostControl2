...
export const PurchasesPage: React.FC = () => {
  ...
  const [editFormData, setEditFormData] = useState({
    date: '', code: '', productName: '', unit: 'kg', customUnit: '', quantity: '', price: '', category: '', supplier: ''
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData((prev) => ({
      ...
      price: '',
      customUnit: '',
      // supplier remains for convenience when entering multiple items from same supplier
    }));
  };
  ...
  const handleExport = () => {
    ...
    const dataToExport = sortedPurchases.map(p => {
      ...
      return {
        ...
        [language === 'ka' ? 'მომწოდებელი' : 'Supplier']: p.supplier || '',
      };
    });
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                  {language === 'ka' ? 'მომწოდებელი' : 'Supplier'}
                </label>
                <input
                  type="text"
                  name="supplier"
                  placeholder={language === 'ka' ? 'მომწოდებლის სახელი' : 'Supplier name'}
                          ...
                          <table className="min-w-full border-collapse text-xs text-left font-mono">
                            <thead className="bg-slate-100 text-slate-700">
                              <tr>
                                ...
                                <th className="border border-slate-300 px-2 py-1.5 uppercase tracking-wide">{language === 'ka' ? 'მომწ.' : 'Suppl.'}</th>
                              ...
                              {datePurchases.map((purchase, index) => {
                                ...
                                return (
                                  <tr key={purchase.id} className="hover:bg-blue-50/50 transition-colors">
                                    ...
                                    <td className="border border-slate-200 px-2 py-1 font-sans text-slate-600 truncate max-w-[100px]">{purchase.supplier || '-'}</td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg flex flex-col transform transition-all">
            ...
            <div className="p-6">
              <form onSubmit={handleEditSubmit} className="space-y-4">
                ...
                {/* === SUPPLIER FIELD IN EDIT MODAL (NEW) === */}
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                    {language === 'ka' ? 'მომწოდებელი' : 'Supplier'}
                  </label>
